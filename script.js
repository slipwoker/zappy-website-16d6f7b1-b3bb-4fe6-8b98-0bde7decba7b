    ...
    if (contactForm) {
        function showError(input, message) {
            ...
            if (existingError) {
                ...
                errorSpan.style.cssText = 'color: #e74c3c; font-size: 0.875rem; margin-top: 0.25rem; display: block;';
        ...
        contactForm.addEventListener('submit', function(e) {
            var isValid = true;
            var nameInput = contactForm.querySelector('[name="name"], #name');
            var emailInput = contactForm.querySelector('[name="email"], #email');
            var phoneInput = contactForm.querySelector('[name="phone"], #phone');
            var messageInput = contactForm.querySelector('[name="message"], #message');
            if (nameInput) {
...
/* SIGNATURE MOMENT: custom-cursor */
(function(){

      (function(){
        if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
        if (window.matchMedia && !window.matchMedia('(pointer: fine)').matches) return;
        var el = document.createElement('div'); el.className = 'zsm-cursor'; document.body.appendChild(el);
        document.addEventListener('mousemove', function(e){ el.style.transform = 'translate(' + e.clientX + 'px,' + e.clientY + 'px) translate(-50%, -50%)'; });
        document.addEventListener('mouseover', function(e){ if (e.target.closest && e.target.closest('a, button, [role="button"], .btn')) el.classList.add('is-link'); });
        document.addEventListener('mouseout', function(e){ if (e.target.closest && e.target.closest('a, button, [role="button"], .btn')) el.classList.remove('is-link'); });
      })();
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '16d6f7b1-b3bb-4fe6-8b98-0bde7decba7b',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '16d6f7b1-b3bb-4fe6-8b98-0bde7decba7b';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '16d6f7b1-b3bb-4fe6-8b98-0bde7decba7b';
